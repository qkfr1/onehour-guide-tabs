# vi. 파이널 문제풀이 → 시험 모드

#### **I. 이런 분들에게 추천해요**

* 내신 시험 직전, 실제 시험과 같은 환경으로 문제를 풀도록 하고 싶은 선생님
* 학원에서 레벨테스트/시험을 운영하여, 최종 점수를 바로 확인하고 싶은 선생님



#### **II. 영상 보며 따라하기**

{% embed url="https://youtu.be/6jUYegG8XiA" %}



#### **III. 이미지 보며 따라하기**

1. **과제를 부여할 문제가 포함된 수업을 선택 후, \[과제 내기] 버튼을 클릭해 주세요.**

<figure><img src="../../../.gitbook/assets/스크린샷 2026-04-22 오후 7.13.01 (1).png" alt=""><figcaption></figcaption></figure>



2. **\[시험 모드] 버튼을 클릭하여 활성화해주세요**

<figure><img src="../../../.gitbook/assets/스크린샷 2026-04-22 오후 7.18.05.png" alt=""><figcaption></figcaption></figure>



3. **과제로 내주실 문제 세트를 클릭 후, \[다음] 버튼을 클릭해 주세요**

<figure><img src="../../../.gitbook/assets/스크린샷 2026-04-22 오후 7.18.30.png" alt=""><figcaption></figcaption></figure>



4. **과제를 내주실 날짜 클릭 후, \[다음] 버튼을 클릭해 주세요**

<figure><img src="../../../.gitbook/assets/스크린샷 2026-04-22 오후 7.18.55.png" alt=""><figcaption></figcaption></figure>



5. **반/학생 선택 후 \[과제 내기 완료] 버튼을 클릭해 주시면 과제 부여가 완료됩니다.**

<figure><img src="../../../.gitbook/assets/스크린샷 2026-04-22 오후 7.19.51.png" alt=""><figcaption></figcaption></figure>



6. **선생님 계정 로그아웃 후 원아워 로그인 페이지로 돌아가서, 학생 아이디/비밀번호 입력 후 \[로그인 하기] 버튼을 클릭해 주세요.**\
   \
   원아워 로그인 페이지 링크 : [https://1hour.ai/kr/login](https://1hour.ai/kr/login)

<figure><img src="../../../.gitbook/assets/스크린샷 2026-04-22 오후 7.20.39.png" alt=""><figcaption></figcaption></figure>





7. **부여한 과제를 클릭 후, \[과제 시작] 버튼을 클릭해 과제를 시작해 주세요.**

<figure><img src="../../../.gitbook/assets/스크린샷 2026-04-22 오후 7.22.49.png" alt=""><figcaption></figcaption></figure>



8. **과제를 수행해보시면 일반 과제와 달리, 문제풀이에 대한 피드백이 나오지 않고 넘어가는 것을 보실 수 있습니다**

<figure><img src="../../../.gitbook/assets/스크린샷 2026-04-22 오후 7.23.34.png" alt=""><figcaption></figcaption></figure>

<figure><img src="../../../.gitbook/assets/스크린샷 2026-04-22 오후 7.23.46.png" alt=""><figcaption></figcaption></figure>



9. **과제 제출 시 바로 과제가 초록색으로 완료 처리되며, 오답 풀기가 불가능해집니다.**

<figure><img src="../../../.gitbook/assets/스크린샷 2026-04-22 오후 7.23.52.png" alt=""><figcaption></figcaption></figure>



